...
- **App logo and icon**: Logo (`resources/logo.png`) and app icon (`resources/icon.ico`) bundled into the repository. The icon is embedded in the exe via `--windows-icon-from-ico` and set at `QApplication` level at runtime. `resources/` is resolved across three candidate base paths (`Path(__file__).parent`, `Path(sys.executable).parent`, `sys.path` entries) so source runs, standalone builds, and onefile builds all find the assets. Windows App User Model ID (`TOFWERK.TWH5Viewer`) is registered before `QApplication` is created.
- **App-wide theme (Preferences dialog)**: Settings → Preferences opens a dialog where the user can choose between "System default" (OS native), "Light (Fusion)", and "Dark (Fusion)". The theme is applied live as the selection changes and persisted via `QSettings`. Cancel reverts the live preview.
- **Keyboard shortcuts**: Ctrl+O (Open file), Ctrl+W (Close file), F1 (Documentation).
- **Help / Documentation menu item**: "Documentation" entry in the About menu (F1) opens the online docs in the system browser.
- **Tooltips**: Descriptive tooltips added to all interactive controls — toolbar / menu actions (Open, Close, File Info, HDF5 Info, Graphing, Imaging, Preferences, Documentation, About), spectrum type selector and copy/export/reset buttons, time-trace channel selector and all associated controls (Update Plot, Auto-update, Time Cursor, Time Range, Mode, Copy, Export, Reset, Buf/Write index spinboxes), data-plot type/X/Y selectors and their copy/export/reset buttons, imaging tool peak and colormap selectors and the Plot Options button, metadata and HDF5 structure search boxes, and the Preferences theme combo.
...
- **File Info — Persistent expansion state**: The set of expanded metadata groups is saved to `QSettings` on every expand/collapse and restored automatically on each file load.
- **HDF5 Info — Structure search**: Live search box above the HDF5 structure tree filters groups, datasets, and attributes by name or info text. Matching paths are auto-expanded; clearing collapses back to root-only.
- **Usage guide expanded**: `docs/usage.md` rewritten to accurately reflect the stacked-tool interface, with new sections for Keyboard Shortcuts, Preferences, HDF5 Structure Browser search, File Info search, metadata channels in the Imaging Tool, and tooltips. Nine screenshots added.

### Known limitations
- **Splash screen (onefile)**: The build script converts `resources/logo.png` to a 24-bit BMP and passes it to `--onefile-windows-splash-screen-image`, but the splash does not appear. Root cause not yet determined; shelved for a future Nuitka version.
- **Taskbar icon (onefile)**: In a Nuitka onefile build the bootstrapper and the Python interpreter run as separate processes. Windows tracks the taskbar button under the bootstrapper's PID; `app.setWindowIcon()` runs in the Python sub-process and has no effect on the taskbar button. The correct icon appears in File Explorer (embedded via `--windows-icon-from-ico`) and in the application title bar.
...
- **Correlation plot — tick label overplotting**: Axis tick labels were duplicated or drawn in a mismatched style in scatter-mode pairplots. Root causes: (1) `fig.subplots()` created all axes independently, so per-column x-ranges and per-row y-ranges were computed separately for each cell, making tick positions inconsistent between cells. Fixed by replacing `fig.subplots()` with manual `fig.add_subplot()` calls that share a single x-axis per column (`sharex`) and a single y-axis per row (`sharey`) for all scatter/histogram cells. Colour-swatch and `'none'` cells remain independent. (2) `apply_theme_to_canvas` called `ax.tick_params(colors=…)` which in some matplotlib versions re-enables `labelbottom`/`labelleft` on shared axes. Fixed by moving the entire tick-label visibility pass to after the theme call. (3) `ax.set_xscale('log')` / `set_yscale('log')` internally resets the tick locator and formatter, which also re-enables hidden labels. Fixed by wrapping the visibility pass in a `_reapply_tick_visibility` closure that is stored on the `MplWidget` and called inside `on_log_x_toggled` and `_on_global_log_y_toggled` after every scale change. Result: only the leftmost column shows y-labels and only the bottom row shows x-labels, consistently in both linear and log scale.
- **Correlation plot — Lin/Log toggles reset on Update Plot**: Clicking "Update Plot" was resetting the Lin X and Lin Y buttons to linear even when the user had switched to log scale. Fixed by capturing the button states before `setup_log_buttons([])` clears `_global_log_mode`, then re-applying the preserved scales to the freshly-created axes before `enable_global_log_toggles` reads them for button synchronisation.
- **Correlation plot — Lin X button not updating text on Update Plot**: After a re-render the Lin X button would retain its previous text (e.g. "Log X") even though the axis scale was reset to linear. Fixed by syncing `btn_log_x` checked state and text from the first applicable axis inside `enable_global_log_toggles`, mirroring the existing sync for `btn_log` (Y).
